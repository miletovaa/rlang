...
For the general analysis I have displayed *top-50 animals by lifespan* and highlighted them by *order*. ***This method will be used further in my analysis for other factors as well.***

```{r}
  plot_top(data, "Order", 50)
```

Graphic shows us that averagely animals of **Primates, Artiodactyla, Carnivora, Squamata** and **Perissodactyla** orders live longer and the absolute long-livers are the representatives of **Testudines, Artiodactyla, Sirenia** and **Proboscidea** orders.

With the further analysis I was trying to explain the patterns of this result. Further analysis included only top long-livers.
...
```{r}
  ggplot(data, aes(x = Average.Lifespan, y = Weight..kg., color = Order)) +
    geom_point() +
    labs(x = "Average Lifespan (years)", y = "Body Mass (kg)")
```

In general it is seen that among long-livers there are animals of very diverse masses. Moreover on the increased scale, a pattern was not detected as well.

```{r}
  mass_plot <- function (scale) {
    ggplot(data %>% filter(!is.na("Weigth..kg.")), aes(x = Average.Lifespan, y = Weight..kg., color = Order)) +
      geom_point() +
  ...
  grid.arrange(mass_plot(5000), mass_plot(1000), mass_plot(100), ncol = 3)
```
...
The initial analysis of top-50 animals by lifespan, categorized by order, unveiled that Primates, Artiodactyla, Carnivora, Squamata, and Perissodactyla tend to have longer lifespans on average, while Testudines, Artiodactyla, Sirenia, and Proboscidea stand out as absolute long-livers.
